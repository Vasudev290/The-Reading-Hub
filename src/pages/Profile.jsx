...
const Profile = () => {
  ...
  const adminStats = user.role === 'admin' ? {
    ...
    borrowedBooksCount: users.reduce((count, u) => 
      count + (u.borrowedBooks?.filter(book => !book.returned).length || 0), 0
    ),
    outOfStockBooks: books.filter(book => book.stock === 0).length,
  } : null;

  // Only show wishlist and borrowed books for regular users
  const wishlistBooks = user.role === 'user' ? books.filter(book => user.wishlist?.includes(book.id)) : [];
  const borrowedBooks = user.role === 'user' ? user.borrowedBooks?.filter(book => !book.returned) || [] : [];
  const returnedBooks = user.role === 'user' ? user.borrowedBooks?.filter(book => book.returned) || [] : [];
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-blue-600 dark:text-blue-400">Total Books</h3>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats.totalBooks}</p>
                </div>
                <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-green-600 dark:text-green-400">Total Users</h3>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats.totalUsers}</p>
                </div>
                <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-yellow-600 dark:text-yellow-400">Borrowed Books</h3>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats.borrowedBooksCount}</p>
                </div>
                <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-red-600 dark:text-red-400">Out of Stock</h3>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{adminStats.outOfStockBooks}</p>
                </div>
              </div>
            </div>
          )}
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                              Borrowed on {new Date(borrowedBook.borrowedDate).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            Returned on {new Date(returnedBook.returnedDate).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </div>
